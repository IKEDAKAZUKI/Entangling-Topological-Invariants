...
from .common import DATA, I2, block_diag, kron3, sx, sy, sz
from . import s4_geometry as geom
from .parameters import S4_COUPLING_ENDPOINT

def coupling_matrices() -> np.ndarray:
...
def structured_mixing_data() -> None:
    ...
    h0 = np.asarray([geom.odd_bundle_hamiltonian(n) for n in nvec])
    v = np.asarray([geom.structured_coupling(n, matrices) for n in nvec])
    ...
    mixed_endpoint = h0 + S4_COUPLING_ENDPOINT * v
    ...
    terms = [
        ...
        ('M0', 'kappa_x', '7 lambda / 40', 'structured interblock pair coupling'),
    ...
    emix = np.linspace(0, S4_COUPLING_ENDPOINT, 21)
    mixing = []
    for value in emix:
    ...
    eps_c2 = np.asarray([0.0, 0.4, 0.8, 1.1, S4_COUPLING_ENDPOINT])
    rows = []
    for value in eps_c2:
        c6, _ = geom.second_chern_quadrature(float(value), 6, matrices)
        c8, gap = geom.second_chern_quadrature(float(value), 8, matrices)
    ...
    for order in (4, 5, 6, 7, 8):
        conv.append([order, *geom.second_chern_quadrature(S4_COUPLING_ENDPOINT, order, matrices)])
    np.savetxt(
        DATA / 'structured_s4_c2_convergence.csv', np.asarray(conv), delimiter=',',
        header='quadrature_order,C2_endpoint,min_quadrature_gap', comments=''
    )
...
def tomography_grid_data() -> None:
    ...
    for order in (4, 5, 6, 8, 10, 12):
        odd = geom.tomography_grid(order, 'odd_mixed', S4_COUPLING_ENDPOINT, matrices)
        even = geom.tomography_grid(order, 'even', 0.0, matrices)
        ch, th, ph, wch, wth, wph, godd, min_no, min_so = odd
        _, _, _, _, _, _, geven, min_ne, min_se = even
        wodd = float(grid_winding_batch(godd[None], ch, th, ph, wch, wth, wph)[0])
        weven = float(grid_winding_batch(geven[None], ch, th, ph, wch, wth, wph)[0])
...
def _odd_hamiltonian_batch(nvec: np.ndarray, epsilon: float, matrices: np.ndarray) -> np.ndarray:
    ...
    derivatives = np.asarray([block_diag(geom.GAMMA[i], zero2, zero2) for i in range(5)])
    constant = block_diag(np.zeros((4, 4), complex), -I2, I2)
    h = constant[None] + np.einsum('ni,ijk->njk', nvec, derivatives)
    v = (
        (7 / 20) * matrices[0][None]
...
def hemisphere_patch_scan() -> None:
    ...
    ref_n, ref_s = geom.tomography_references('odd_mixed', S4_COUPLING_ENDPOINT, matrices)
    ...
    def scan(vectors: np.ndarray, reference: np.ndarray):
        ...
        for start in range(0, len(vectors), 4096):
            ...
            h = _odd_hamiltonian_batch(nv, S4_COUPLING_ENDPOINT, matrices)
...
def batch_patch_frame(p: np.ndarray, reference: np.ndarray):
