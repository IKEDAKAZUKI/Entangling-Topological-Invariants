...
from eti.parameters import LABEL_MIXING_ENDPOINT, S4_COUPLING_ENDPOINT
...
def check_reference_values() -> dict[str, float]:
    mixing = load_named("label_mixing_curvature_integrals.csv")
    row = row_at(mixing, "epsilon", LABEL_MIXING_ENDPOINT)
    ...
    c2_row = row_at(c2, "lambda", S4_COUPLING_ENDPOINT)
...
def main() -> None:
    csv_count = check_csv_schemas()
    values = check_reference_values()
    figure_count = check_figures()
    print(f"Checked {csv_count} CSV files and {figure_count} figures against the distributed outputs.")
